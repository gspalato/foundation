...
const Component: React.FC = () => {
	...
	const fetch = async () => {
		await FoundationClient.GetRefillStationResumes();

		const response = await FoundationClient.GetRefillStationResumes();
		const data = await response.json();

		if (!response.ok) {
			console.log('Failed to fetch data from Foundation:');
			console.log(data);
			return;
		}
